...
class DoSCM(torch.nn.Module):
    def __init__(
        ...
        seed: int,
        noise_std: float,
        noise_dist: Literal["gaussian", "laplace", "student", "gumbel"],
        nonlins: Literal["post", "mixed", "tanh", "sin", "neg", "id", "elu", "summed", "sophisticated_sampling_1", "sophisticated_sampling_1_normalization", "sophisticated_sampling_1_rescaling_normalization"],
        exo_std: float,
        zero_one_treatment: bool,
        binary_strategy: Literal["mean", "extreme"],
        inference_cov: Literal['pre_interventional', 'pre_treatment'],
        graph: nx.DiGraph | None,
        t_idx: int | None,
        y_idx: int | None,
        x_idcs: tuple[int, ...] | None,
        max_hidden_layers: int = 1, # unused
    ):
        ...
        with torch.no_grad():
            ...
            self.zero_one_treatment = zero_one_treatment
            self.graph = graph
            self.x_idcs = x_idcs
            self.t_idx = t_idx
            self.y_idx = y_idx
            self.inference_cov = inference_cov
            self.binary_strategy = binary_strategy

            self.exo_dist = "gaussian" # TODO: No other supported
    ...
    def forward(self):  
        ...
        self.scm.binary_strategy = self.binary_strategy
        ...
        if self.inference_cov == 'pre_interventional' or self.inference_cov == 'pre_treatment':
            ...
            coin_flips = torch.randint(0, 2, (self.batch_size, self.num_samples))
...
def get_batch(
    ...
    **kwargs,
    ...
    print("Unused kwargs:", list(kwargs.items()))
    do_scm = DoSCM(
        ...
        exo_std=exo_std,
        zero_one_treatment=zero_one_treatment,
